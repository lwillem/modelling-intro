...
To begin the health technology assessment, we calculate the vaccine cost per dose and the treatment cost per case, considering the proportion of cases that result in hospitalization, the QALY lost per case and the discounted life expectancy:
```{r }
...
# Cost-effectiveness analysis when accounting for uncertainty (using BCEA package).
...
```{r }
#Prepare and run the 'bcea()' function
c=cbind(comparator_totaldirectcosts,program_totaldirectcosts)
e=cbind(-comparator_QALYslost_disc,-program_QALYslost_disc) ##negative sign because input needs to be health gain (and we are loosing health due to flu)
ints=c("No intervention","Vaccine program")
m <- bcea(e,c,ref=2,interventions=ints,plot=T) #ref specifies to compare 'Vaccine program' to 'No intervention'

```

2. Which decision should I take, given current information and for a range of WTP values? Print the summary of the `bcea()` function:
```{r }
summary(m)
```
