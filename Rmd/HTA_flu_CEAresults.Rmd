...
# print mean results
round(colMeans(ICER_values))

# print subset of the simulation results
head(round(ICER_values))
```
...
mean(incr_total_costs)/mean(infections_averted) # ratio of means
```
...
for (i in 1:Nsamples)
{discounted_life_expectancy[i]<-sum(1/((1+discountrate)^(0:(life_expectancy[i]-1))))}
...
hist(force_of_infection)
hist(population_at_risk)

hist(comparator_infections)
mean(comparator_infections)
