...
if (!exists("show_results")) show_results = FALSE
...
The goal is to calculate the cost-effectiveness of a vaccination program for the elderly against flu, taking into account disease burden, discounting, QALYs and parameter uncertainty as specified below:
...
treatment_cost_per_case = cost_per_hosp_case * p_hosp + (1 - p_hosp) * cost_per_non_hosp_case
QALYloss_per_case       = QALYloss_hosp_case * p_hosp + QALYloss_non_hosp_case * (1 - p_hosp)
...
# Program
population_at_risk = targetgroup * (1 - uptake_program) +
                     targetgroup * uptake_program * (1 - vaccine_efficacy)
...
comparator_vaccination_cost   = 0 # for ilustration purpose, no vaccine in the comparator
...
incr_vaccination_cost      = program_vaccination_cost - comparator_vaccination_cost # incremental vaccine cost
...
Kindly review the following plots and estimates:
...
# Plot the histograms
hist(incr_total_cost,
     xlab = 'Incremental total cost',
     main = 'Histogram of Incremental Total Cost')
abline(v = mean(incr_total_cost), col = 2)
hist(QALY_gained_disc,
     xlab = 'QALY gain (discounted)',
     main = 'Histogram of QALY Gain (discounted)')
abline(v = mean(QALY_gained_disc), col = 2)
hist(incr_total_cost / QALY_gained_disc,
     xlab = 'point-wise incremental total cost per infection averted',
     main = 'Histogram of Pointwise Incremental Cost per Infection Averted')
...
# Try WTP values bracketing the ICER to find where the expected INMB changes sign.
# At the ICER, the expected INMB is exactly 0. Values above the ICER yield positive
# expected INMB (intervention is cost-effective); values below yield negative INMB.
k = 8000 # above the ICER: expected INMB should be positive
mean(QALY_gained_disc * k - incr_total_cost)

# the INMB is not affected by the order of the operations
mean(QALY_gained_disc) * k - mean(incr_total_cost)

k = 6000 # below the ICER: expected INMB should be negative
mean(QALY_gained_disc * k - incr_total_cost)
...
# plot CE plane
plot(x = QALY_gained_disc,
     y = incr_total_cost,
     xlab = 'Incremental effect (QALY gain)',
     ylab = 'Incremental cost (euro)',
     main = 'CE plane')

```

# Present CEA (w/o R package).

```{r}

# set WTP range, and step size
opt_wtp = seq(0, 40000, 500)

# initialise matrix
inmb_wtp_summary = data.frame(wtp              = opt_wtp,
                              inmb             = NA,
                              ceac             = NA,
                              ceaf             = NA,
                              inmb_perfect_info = NA)

# loop over all WTP levels
for(i in 1:length(opt_wtp)){
  # calculate the inmb per sample with the current WTP threshold
  inmb_sample = (QALY_gained_disc * opt_wtp[i]) - incr_total_cost

  # get the expected INMB
  inmb_wtp_summary$inmb[i] = mean(inmb_sample)

  # inspect the number of samples with a positive INMB
  inmb_wtp_summary$ceac[i] = sum(inmb_sample >= 0) / nsamples

  # to calculate INMB with perfect information, set all non-preferred options to zero
  # note: if there are more than 2 options, other code is needed
  if(inmb_wtp_summary$inmb[i] >= 0){
    inmb_sample[inmb_sample < 0] = 0
  } else{
    inmb_sample[inmb_sample > 0] = 0
  }
  # get the expected INMB with perfect information
  inmb_wtp_summary$inmb_perfect_info[i] = mean(inmb_sample)
}

# get preferred strategy by wtp, in this case, when INMB is positive
inmb_wtp_summary$ceaf[inmb_wtp_summary$inmb > 0] = inmb_wtp_summary$ceac[inmb_wtp_summary$inmb > 0]

# net-loss: difference between INMB under perfect information and expected INMB.
# This equals the per-patient EVPI: the maximum a decision maker should pay to
# eliminate all parameter uncertainty before choosing between strategies.
inmb_wtp_summary$net_loss = abs(inmb_wtp_summary$inmb_perfect_info - inmb_wtp_summary$inmb)

```

Inspect the CEA outcomes if the WTP threshold is 10000.

```{r}

inmb_wtp_summary[inmb_wtp_summary$wtp == 10000, ]

```


Inspect the included WTP for which the intervention becomes cost-effective. Compare this with the ICER.

```{r}

# summary statistics
ce_threshold = min(inmb_wtp_summary$wtp[inmb_wtp_summary$inmb > 0])
icer         = mean(incr_total_cost) / mean(QALY_gained_disc)

data.frame(ce_threshold = ce_threshold,
           icer         = icer)
...
# CEAC
plot(x = inmb_wtp_summary$wtp,
     y = inmb_wtp_summary$ceac,
     type = 'b',
     xlab = 'Willingness To Pay (euro)',
     ylab = 'Probability to be cost-effective',
...
Or you can combine the CEAC and CEAF

```{r}

# Combination of CEAC & CEAF
plot(x = inmb_wtp_summary$wtp,
     y = inmb_wtp_summary$ceac,
     type = 'b',
     lty = 3,
...
plot(x = inmb_wtp_summary$wtp,
     y = inmb_wtp_summary$net_loss,
     type = 'b',
     xlab = 'Willingness to pay (euro)',
     ylab = 'Expected net-loss (euro)',
...
If we are interested in the EVPPI, hence per individual parameter of the influenza model above, we can adopt the GAM approximation using the calculated incremental cost and benefit with all sampled parameter values.
...
# Present CEA (using the `BCEA` package).
...
Once installed, you have to load the package(s) using
...
# inspect the documentation of the 'bcea()' function: ?bcea
...
`r if (!show_results) {
    '<a href="CEA_flu_uncertainty_results.html"> Results</a>'
  } else {
    '<a href="CEA_flu_uncertainty.html"> Back</a>'
  }
`
</div>
