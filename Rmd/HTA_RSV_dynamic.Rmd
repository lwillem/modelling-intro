...
morbidity_per_episode     = 0.002196 # QALY
proportion_mortality      = 0.006    # assume age-invariant
average_life_expectancy   = 9        # QALY
...
5. **The Role of Uncertainty:** Reflect on the decision when the cost per case is represented by `rgamma(100, shape=(122/50)^2, rate = 122/50^2))` and the vaccine efficacy by `rnorm(100,0.4,0.1)`. Try to reproduce the figure below for 100 samples:
...
num_samples <- 100
set.seed(1234)
unc_cost_per_episode <- rgamma(num_samples, shape=(122/50)^2, rate = 122/50^2)
...
unc_static_icer            = unc_static_incr_cost  / unc_static_incr_qaly
...
unc_dynamic_icer            = unc_dynamic_incr_cost  / unc_dynamic_incr_qaly
