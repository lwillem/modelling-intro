---
title: "Introduction to individual-based models (IBMs) for infectious diseases"
---
...
# Objectives
The aim of this tutorial is to introduce you to individual-based modelling for infectious diseases in R. The dynamics of the stochastic model we target here cannot be captured by ordinary differential equations.

In this session, you will

1. Setup an individual-based model population with random movement
2. Include SIR-dynamics 
3. Combine all features into a loop to run over multiple time steps
...
* To create a population matrix (or data.frame) with one row per individual and one column per person characteristic
* Add random walk (with borders)
* Add health states for being susceptible (S), infected (I) and recovered (R)
* Include transmission and recovery dynamics
...
In R, you can generate random numbers using e.g. `sample`, `runif`, `rnorm`,`rbinom`. Please have a look at:
```{r results = "hide"}
?sample
?runif
?rnorm
...
Random numbers in R (and other programs) are derived from a "random number stream", which you can initiate with `set.seed`. Seeding the random stream is very useful to obtain stochastic results that are reproducible. As such, you can check whether model output has changed (or not) if you continue programming.
...
Now, let us create a `data.frame` for a population of 1000 individuals with a health state together with a x- and y coordinate. During the setup, we randomly locate each person in a 10x10 area.
```{r }
...
# set max distance per time step 
max_distance <- 1   # squares per day

random_walk <- function(pop_data, max_distance){
  # step 1: move each person at random along the x and y axis
  pop_data$x_coord <- pop_data$x_coord + runif(n = pop_size,min = -max_distance, max = max_distance)
  pop_data$y_coord <- pop_data$y_coord + runif(n = pop_size,min = -max_distance, max = max_distance)
...
# plot the area, using all locations with color '1' => black
...
# update population
pop_data <- random_walk(pop_data,max_distance)
...
We assume that susceptible individuals can acquire an infection if they are close to an infected individual. We define proximity here with a maximum distance in terms of `contact_range`. To probability for a social contact, depends on the number people that are within the specified range and the population-based average number of social contacts individuals have per time step (=per day). 

```{r}

# set proximity parameter and the population-based average number of contacts per day
contact_range <- 1
avg_num_contacts_day <-  5
...
eligible_contacts_matrix <- distance_matrix <= contact_range
...
Now, we will write a function to count the number of individuals that are close enough, and calculate the probability for a social contact with each of them. Please note that Booleans can be summed, using `r TRUE` as 1 and `r FALSE` as 0.
...
get_contact_probability <- function(eligible_contacts_matrix,index_person){
  ...
  contact_probability <- avg_num_contacts_day / num_eligible_contacts
...
# set number of infected cases to start the simulation
num_infected_cases <- 10

# or select all individuals from row 1 till the specified number of cases.
pop_data$health[1:num_infected_cases] <- 'I'

## it is also possible to sample random individuals to change their health state
#pop_data$health[sample(1:pop_size,size = num_infected_cases)] <- 'I'
...
Can you visualize the infected individuals? (note: you might have different results since we are working with stochastic processes)
```{r echo=FALSE}
...
Now you have a working model, you can start changing input parameters and explore the effect on the transmission dynamics. E.g. what happens if you...
...
(note: the next tutorial will handle these questions with a pre-coded model)

# Extending the SIR model

If you have time, you can now try to build more complex versions of the SIR model to match the infectious disease you are interested in. For example, you could include a latent (or "Exposed") stage or extend the model to include waning immunity, vaccination, maternal immunity, etc. 


<div>
# Navigate
Top: [Index](index.html) Next: [IBM scenario analyses](intro_to_IBM_scenario.html)
</div>
