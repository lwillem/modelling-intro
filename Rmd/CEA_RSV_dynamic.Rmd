...
dynamic_cases_reference = diff(dynamic_rsv_reference$results$R[c(1, nrow(dynamic_rsv_reference$results))]) * 
                            population_size
...
db_out = data.frame(model                = c('static', 'dynamic'),
                    ...
                    program_icer         = c(static_icer, dynamic_icer))
...
# As such, the CEA results are (more) in line with the CEA based on the static
# disease model.
...
`r if (!show_results) {
    '<a href="CEA_RSV_dynamic_results.html"> Results</a>'
  } else {
    '<a href="CEA_RSV_dynamic.html"> Back</a>'
  }
`
</div>
