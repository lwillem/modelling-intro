...
get_default_parameters = function(){
  ...
  attach(list(pop_size = 1000,  # population size
              ...
              return_prevalence       = FALSE,  # option to return the prevalence (and stop)
...
run_ibm_random_walk = function(pop_size = 1000,  # population size
  ...
  if(!is.logical(c(apply_spatial_vaccine_refusal, add_baseline, return_prevalence))){
    warning("ERROR: 'apply_spatial_vaccine_refusal', 'add_baseline' and 'return_prevalence' should be a boolean")
    return(NULL)
  }
  ...
  if(add_baseline){
    out_baseline = run_ibm_random_walk(rng_seed = rng_seed, return_prevalence = T)
...
sample_vaccine_refusal = function(pop_data, vaccine_coverage){
  ...
  # if we have too little vaccine potentials, add random individuals
  ...
  # sample from the potential vaccinated individuals
...
#'
#' @description This function shows the spatial configuration of the population
#' with color codes for the health state and the social contact radius of one individual.
#'
#' @param pop_data             the vector with population data
#' @param area_size            the total area size
...
geo_plot_social_contact_radius = function(pop_data, area_size, max_contact_distance, average_num_contacts, num_days)
{
  ...
  # add grid lines
  # note: 'abline' covers the full figure area and cannot be stopped at the model world boundary
...
run_ibm_location = function(pop_size              = 2000,     # population size
  ...
  if(!is.logical(c(bool_show_demographics, add_baseline, return_prevalence))){
    warning("ERROR: 'bool_show_demographics', 'add_baseline' and 'return_prevalence' should be a boolean")
    return(NULL)
  }
  ...
  if(add_baseline){
    out_baseline = run_ibm_location(rng_seed = rng_seed, return_prevalence = T, bool_show_demographics = F)
    lines(out_baseline$log_i, col = 2, lwd = 2, lty = 2)
    legend('topright', legend = c('I (baseline)'), col = 2, lwd = 2, lty = 3, cex = 0.7, bg = 'white')
  } else{
    out_baseline = NA
...
create_population_matrix = function(pop_size, num_schools, target_school_ages, num_workplaces, bool_show_demographics = TRUE)
{
  ...
  if(bool_show_demographics){
    ...
    if(any(!is.na(pop_data$workplace_id))) hist(table(pop_data$workplace_id), xlab = 'Size', main = 'Workplace size')
  }
